...
- **Derslerim'in "Ders araçları" bölümünden açılan:** `student/teacher` (Öğretmenlerim: yalnızca bağlı öğretmen(ler)in bilgi kartı — dersler Derslerim'de, ödev/not/mesaj "yakında"). **Çalış'ın kısayollarından açılan:** `student/goals-overview` (**Hedefler**: günlük/haftalık/net ilerleme + düzenleme).
