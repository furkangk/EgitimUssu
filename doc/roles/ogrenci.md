...
- **Sekmeler (StudentBottomNav, 5 — 5-sekme IA, 2026-07-21):** `student-home` (**Çalış** — çalışma panosu, ux bilgi hiyerarşisine göre sıralı: motivasyon alt başlığı + yan yana bugünkü çalışma/günlük seri kartları + hızlı işlemler + yaklaşan ders + bu hafta/rekor + son çalışma/deneme), `student/performance` (**Performans** — Task 1'de yalnız rota/nav taşındı, içerik hâlâ eski Testler ekranı `student_tests_page.dart`: deneme gir + net trend grafiği + ders bazlı analiz + son denemeler), `student/lessons` (**Derslerim** — Task 1'de yalnız rota/nav taşındı, içerik hâlâ eski Takvim ekranı `student_calendar_page.dart`: birleşik ders programı — öğretmen dersleri salt-okunur/öncelikli + öğrencinin kendi oluşturduğu dersler; ay takvimi (`SfCalendar`) → seçili gün listesi; tek/tekrarlı kişisel ders ekle/düzenle/sil, öğretmen slotuna eklenemez), `student/discover` (**Keşfet** 🔴 Faz 4 yer tutucu — işlevsel arama yok, `student_discover_page.dart`), `student/profile` (**Profil** — artık ayrı sekme, ux §11: toplam çalışma/gün/rekor + toplam deneme/net + en çok çalışılan ders + rozet özeti). Eski rotalar geri-uyum redirect'i: `student/tests`→`student/performance`, `student/calendar`→`student/lessons`.
- **Sekme olmaktan çıkan, henüz push ile erişilebilen ekranlar (route/sayfa Task 3/6'da retire edilecek):** `student/studies` (eski Çalışmalarım: sayaç + haftalık grafik + derslere göre süre + istatistik + son çalışmalar), `student/more` (Diğer hub — Rozetler, Öğretmenlerim, Hedefler, Hedef & paylaşım, Hesap, Çıkış).
- **Diğer hub'ından açılan (push, değişmedi):** `student/teacher` (Öğretmenlerim: yalnızca bağlı öğretmen(ler)in bilgi kartı — dersler Derslerim'de, ödev/not/mesaj "yakında"), `student/goals-overview` (**Hedefler**: günlük/haftalık/net ilerleme + düzenleme).
