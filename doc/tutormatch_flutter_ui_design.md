...
# EgittimUssu Tab Widget Tasarım Dokümanı

Bu doküman, EgittimUssu mobil uygulama tasarımında kullanılan **Tab / Segment Control Widget** bileşeninin tasarım, kullanım ve Flutter implementasyon kurallarını açıklar.
...
EgittimUssu tasarımında tab yapısı özellikle şu ekranlarda kullanılır:
...
EgittimUssu tab widget tasarımı:
...
Aktif tab, EgittimUssu primary rengi olan koyu lacivert ile gösterilir.

```dart
backgroundColor: Color(0xFF062B52)
...
```dart
class AppColors {
  static const primary = Color(0xFF062B52);
...
class EgittimUssuTabBar extends StatelessWidget {
  final List<String> tabs;
  final int selectedIndex;
  final ValueChanged<int> onChanged;

  const EgittimUssuTabBar({
    super.key,
    required this.tabs,
...
```dart
class EgittimUssuTabItem {
  final String label;
  final int? badgeCount;

  const EgittimUssuTabItem({
    required this.label,
    this.badgeCount,
  });
}
...
textColor: Color(0xFF062B52)
```

---

## 14. Component API Önerisi
...
```dart
EgittimUssuSegmentedTabs(
  items: const [
    EgittimUssuTabItem(label: 'Aktif', badgeCount: 4),
    EgittimUssuTabItem(label: 'Tamamlanan', badgeCount: 12),
  ],
  selectedIndex: selectedIndex,
  onChanged: (index) {},
)
...
EgittimUssu tab widget, uygulamanın genel sade ve profesyonel dilini destekleyen küçük ama kritik bir navigasyon bileşenidir.
